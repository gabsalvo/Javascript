...
async function isUserRegistered(uid: string) {
...
const loginWithGoogle = async () => {
  try {
    ...
    if (await isUserRegistered(user.uid)) {
      ...
      alert("Per favore, registra un account prima di effettuare il login.");
    }
  } catch (error) {
    if (error instanceof Error) {
...
<button on:click={registerWithGoogle} disabled={!usernameGoogle}>Register with Google</button>
